...
# Build the CleverSwitch macOS release archive: a PyInstaller onefile binary
# packed with the docs, config example, and install scripts.
# Output: dist/cleverswitch_macOS.tar.gz

APP_NAME="cleverswitch"
ARCHIVE="cleverswitch_macOS"
...
# This is exactly how the broken v1.2.5 release shipped. `pip install .` pulls
# in pyyaml plus the macOS-only bleak + pyobjc.
...
# ── Step 2: PyInstaller ──────────────────────────────────────────────
# PyInstaller follows dylib dependencies via otool -L automatically — no
# --add-binary needed. --hidden-import yaml is a safety net.
