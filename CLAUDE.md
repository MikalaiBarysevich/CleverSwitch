...
`PeerHostFollowSubscriber` is the fallback for that case: a pragmatic, explicitly *opt-in* mechanism for the common two-machine setup, config-driven rather than protocol-driven. `easy_switch.peer_host_index` (`EasySwitchConfig`, keyed by device role - see `config.example.yaml`) is a fixed, user-learned Easy-Switch host number for "the other machine". Since there's no way to distinguish a genuine Easy-Switch press from an ordinary RF dropout on the wire, this subscriber treats *any* disconnect of either device - while its paired counterpart is still locally connected - as "it switched away", and commands the counterpart to follow to its own role's configured index. It's deliberately symmetric in both directions (mouse-initiated switches also bring the keyboard along), unlike Logitech's own keyboard-only Enhanced Easy-Switch, and it's an *alternative*, not a replacement, for the notification-based flow above - both subscribers are always wired up; `ChangeHostNotificationSubscriber` (and the CID path) still fire first whenever the underlying notification does exist, and `PeerHostFollowSubscriber` stays inert (empty `peer_host_index`) unless a user explicitly configures it. Per-role rather than one shared index, because each device keeps its own independent host pairing table - two paired devices' index for "the same other machine" aren't guaranteed to match. Guards against the gateway's *startup* enumeration (which reports every device already paired elsewhere as "disconnected") by only reacting to a disconnect that follows a locally-observed connect within the daemon's own process lifetime - otherwise every restart would misfire the relay for whichever device isn't on this machine right now.
