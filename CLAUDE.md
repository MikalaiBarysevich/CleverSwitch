...
Active subscribers: `DeviceConnectionSubscriber`, `DeviceInfoSubscriber`, `InfoTaskOrchestrator`, `SetReportFlagSubscriber`, `ExternalUnsetFlagSubscriber`, `AnalyticsRejectionSubscriber`, `HostChangeSubscriber`, `ChangeHostNotificationSubscriber`, `PeerHostFollowSubscriber`, `EventHookSubscriber`, `WirelessStatusSubscriber`, `TransportDisconnectionSubscriber`.
...
`ChangeHostNotificationSubscriber` covers devices (e.g. MX Keys S) whose ES CIDs never deliver an x1B04 notification at all: on Easy-Switch press they instead emit an undocumented feature 0x1814 fn=0/sw_id=0 notification on the departing host (`payload[0]`=departing host, `payload[1]`=target host, both 0-based). Since the parser is stateless and can't map a feature index back to a feature code, the match against the device's resolved x1814 index happens in this subscriber; on a match it publishes the same `HostChangeEvent` as the CID path above, so `HostChangeSubscriber` handles both uniformly.

**That 0x1814 notification is not universal.** On some MX Keys S + Bolt-receiver pairings it never fires at all - confirmed via raw HID capture on both Linux and macOS (`-vv`, comparing every `Received HID event` hex against every `Parsed event` log line): the departing host receives nothing but the bare 0x41 disconnect report, no target-host information whatsoever, even across a full reconnect + rediscovery cycle. `ChangeHostNotificationSubscriber` has nothing to react to in that case, and the underlying protocol offers no alternative signal - the departing host is the *only* place a target-host announcement could ever arrive (a device that has switched away tears down its RF/BT link entirely; see `HostChangeSubscriber`'s CID-path comment for why there's no cross-device HID++ channel), so this isn't fixable by listening differently or waiting longer.

`PeerHostFollowSubscriber` is the fallback for that case: a pragmatic, explicitly *opt-in* mechanism for the common two-machine setup, config-driven rather than protocol-driven. `easy_switch.peer_host_index` (`EasySwitchConfig`, keyed by device role - see `config.example.yaml`) is a fixed, user-supplied Easy-Switch host number for "the other machine", simply read off that device's own Easy-Switch keys. Since there's no way to distinguish a genuine Easy-Switch press from an ordinary RF dropout on the wire, this subscriber treats *any* disconnect of either device - while its paired counterpart is still locally connected - as "it switched away", and commands the counterpart to follow to its own role's configured index. It's deliberately symmetric in both directions (mouse-initiated switches also bring the keyboard along), unlike Logitech's own keyboard-only Enhanced Easy-Switch, and it's an *alternative*, not a replacement, for the notification-based flow above - both subscribers are always wired up, and `PeerHostFollowSubscriber` stays inert (empty `peer_host_index`) unless a user explicitly configures it. Per-role rather than one shared index, because each device keeps its own independent host pairing table - two paired devices' index for "the same other machine" aren't guaranteed to match. `_parse_peer_host_index` validates both the role key (against `VALID_ROLES` in `model/logi_device.py`) and the 1-based number (against `VALID_HOSTS` in `hidpp/constants.py`, moved there from `ChangeHostNotificationSubscriber` so config and the notification path share one source of truth); either mistake would otherwise leave the relay silently doing nothing.

Guards against the gateway's *startup* enumeration (which reports every device already paired elsewhere as "disconnected") by only reacting to a disconnect that follows a locally-observed connect within the daemon's own process lifetime - otherwise every restart would misfire the relay for whichever device isn't on this machine right now.

**Disconnect filtering: debounce + activity gating.** A relay-worthy disconnect is indistinguishable on the wire from two kinds of false positive, each with its own filter. (1) RF micro-dropouts during active use (measured: the dropped device reconnects within p50 1.0s / max <2s): the relay publish is deferred by `RECONNECT_DEBOUNCE_S` (2.0s) and cancelled if the departed device reconnects inside the window, via a per-wpid generation counter bumped on every reconnect. (2) Idle power-save link drops (observed on MX Keys S as 0x41 link-down after inactivity, with no reconnect for 13-41s - beyond any debounce that wouldn't also swallow real switches): when an `InputActivityMonitor` is injected, a disconnect is only treated as a departure if the device produced input within `ACTIVITY_WINDOW_S` (30s) or its link was younger than `FRESH_LINK_S` (5s, the wake→switch shape: a slept device woken just to press ES reports connect then immediate disconnect with no input in between - the ES key itself is consumed by firmware and never reported; that raw-capture finding is also why no press-notification path exists at all on this pairing). The gate **fails open** whenever activity data is missing (no monitor, fresh start, or a platform that denies opening the input collection - Windows denies keyboard/mouse top-level collections): a wrong suppression silently breaks real switches, a wrong relay is the old status quo. The raw 0x41 payload cannot help instead: a dropout carries `r1 = 0x40 | device_type`, byte3 = 0x10 - bit-identical to any other link-down.

`InputActivityMonitor` (`monitor/input_activity_monitor.py`) supplies the timestamps: one reader thread per standard keyboard/mouse input collection of Logitech devices (`enumerate_input_collections` in `hidpp/transport.py`, Generic Desktop usages 0x06/0x02 mapped to roles), each report refreshing a monotonic timestamp keyed by (pid, role) - contents never inspected. Deliberately not a Topic publisher (a moving mouse is 50+ reports/s); injected shared state like the registry and cache, queried on demand. A manager thread re-enumerates every 5s and reopens collections whose readers died (receiver unplug surfaces as TransportError). Only constructed at all when `easy_switch.peer_host_index` is non-empty (`_setup_activity_monitor` in `app_setup.py`) - the daemon stays away from input collections unless the user opted into the relay that needs them. **`hid_enumerate` is not thread-safe** (hard abort in libudev when the discovery loop and the monitor's sweep enumerate concurrently) - all enumeration goes through `_enumerate_lock` in `transport.py`.

Because the relay now fires ~2s after the disconnect, the announced-switch suppression check is anchored to the disconnect timestamp rather than the timer-fire time - an announcement suppresses if it landed no earlier than `ANNOUNCED_SWITCH_WINDOW_S` before the disconnect, and any announcement arriving during the debounce window suppresses too.

**Two mechanisms, one command.** Because both relays are always wired up, a device whose announcement *does* work would otherwise be relayed twice per press - once to the announced target, then again to the configured peer index when the disconnect lands - and on a three-host setup the second command overrides the correct one. `PeerHostFollowSubscriber` therefore suppresses itself for `ANNOUNCED_SWITCH_WINDOW_S` (1.0s, sized from measured device link-teardown latency - see the constant's comment) after any `HostChangeEvent` it did not publish itself (identity check against `_relayed_event`; `Topic.publish` fans out the same object, so this is exact). Suppression is *not* keyed per device: the announcement and the disconnect to ignore can come from different devices - the departing device announces, `HostChangeSubscriber` commands its counterpart to follow, and then the counterpart's own disconnect must not fire a second relay.

**Why the relay is deferred.** The CID path emits `HostChangeEvent` straight from the parser, so FIFO ordering on `hid_event` guarantees the suppression signal is seen before the disconnect. The 0x1814 path does not: the parser emits a generic `HidppNotificationEvent` and `ChangeHostNotificationSubscriber` translates it on *its own* thread, so its `HostChangeEvent` can lose the race to the disconnect. Losing it is not self-correcting - the relay would command the peer to the wrong host, the peer would tear down its link, and the correct command arriving afterwards would never reach it. So the publish is deferred on a `threading.Timer` (`RECONNECT_DEBOUNCE_S`, which also serves as the dropout debounce above; `relay_delay_s` is injectable for tests), and the timer callback re-checks `peer.connected`. It cannot be a plain `sleep` in `notify()` - the announcement arrives on this subscriber's own queue, which its drain thread would then not be draining. This makes `_announced_at` / `_relayed_event` cross-thread state, hence the lock.

A receiver unplug also fires the relay: `TransportDisconnectionSubscriber`'s fan-out runs on its own thread, so the peer's `connected` may still be True when the first device's disconnect is handled. The `CHANGE_HOST` write is dropped by the gateway (transport gone), but the INFO line is logged and `SWITCH` hooks do fire - documented in `README.md` and `config.example.yaml` because a hook can perform real work.

**Known limitation: the relay's own command echoes back as a second relay.** Confirmed on live hardware (`13:51:46` → `13:51:47` in a Bolt-receiver capture). The relay's `CHANGE_HOST` makes the peer tear down its link, and a device tearing down its link is indistinguishable from a departure - so the peer's disconnect fires the relay again, commanding the originally-departed device. `peer.connected` is supposed to stop this and does not: `DeviceConnectionSubscriber` applies `connected` on its own thread from the same fanned-out event, so `PeerHostFollowSubscriber` can still read the peer as connected roughly a second after the peer's disconnect was published. Measured link-teardown latency after a `CHANGE_HOST` is p50 0.71s / max 1.87s (n=37), which is the size of that window.

Harmless on two hosts, where both relays name the same target and the second is a no-op repeat. **Wrong-machine switch on three or more hosts:** relay #1 correctly follows an announced target of host 3, then relay #2 overrides it with `peer_host_index`, which can only ever name one fixed "other machine" - and the last command to land wins. Documented in `README.md` under "Known limitation" with the recommendation to leave `peer_host_index` unset beyond two machines.

A proper fix cannot be a time window (it would swallow genuine departures during rapid switching, which is a real usage pattern). It needs the relay to mark the peer as "commanded away" and consume exactly one of its disconnects, cleared on reconnect - plus a bounded expiry as a backstop, because a `CHANGE_HOST` naming the host a device already sits on may produce no disconnect at all and would otherwise leave the flag armed against the next genuine departure.
