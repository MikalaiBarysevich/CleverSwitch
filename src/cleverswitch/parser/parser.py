...
from ..hidpp.constants import HOST_SWITCH_CIDS, MAP_FLAG_DIVERTED, REPORT_LONG, REPORT_SHORT, SW_ID
...
def parse(pid: int, raw_event: bytes) -> Event | None:
    ...
    if report_id == REPORT_SHORT and feature_id == 0x41:
        ...
        return DeviceConnectedEvent(slot=slot, pid=pid, link_established=link_established, wpid=wpid,
                                    device_type=device_type)

    # HID++ 1.0 error (sub_id=0x8F)
    if report_id == REPORT_SHORT and feature_id == 0x8F:
        error_code = raw_event[5]
        return HidppErrorEvent(slot=slot, error_code=error_code, pid=pid)
    ...
    if report_id == REPORT_LONG:
        ...
        if sw_id == SW_ID:
            payload = raw_event[4:]
            return HidppResponseEvent(slot=slot, pid=pid, feature_index=feature_id, function=function, payload=payload)

        # Notification from device (sw_id == 0)
        if sw_id == 0:
            payload = raw_event[4:]
            return HidppNotificationEvent(slot=slot, pid=pid, feature_index=feature_id, function=function, payload=payload)
