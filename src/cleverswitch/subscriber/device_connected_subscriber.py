...
class DeviceConnectinSubscriber(Subscriber):
    ...
    def notify(self, event) -> None:
        ...
        if event.link_established:
            log.info(f"Device connected: slot={event.slot}, wpid=0x{event.wpid:04X}")
        else:
            log.info(f"Device disconnected: slot={event.slot}, wpid=0x{event.wpid:04X}")
            return

        if logi_device is not None:
            if logi_device.info_step < 5:
                log.info(f"Device reconnected with incomplete setup (step {logi_device.info_step}/5): wpid=0x{event.wpid:04X}")
                ...
                log.debug(f"Device already registered: {logi_device}")
            return
