...
class FeatureTask(InfoTask):
    ...
    def doTask(self) -> None:
        if self._feature_code in self._device.available_features.values():
            log.debug(f"Feature 0x{self._feature_code:04X} already resolved")
        ...
        if event is None:
            log.warning(f"Timeout resolving feature 0x{self._feature_code:04X} for wpid=0x{self._device.wpid:04X}")
            return

        if isinstance(event, HidppErrorEvent):
            log.warning(
                f"Error resolving feature 0x{self._feature_code:04X} for slot={self._device.slot}: "
                f"error=0x{event.error_code:02X}"
            )
            return
        ...
        if event.payload[0] != 0x00:
            ...
            log.debug(f"wpid=0x{self._device.wpid:04X}: feature 0x{self._feature_code:04X} at index {feat_idx}")
        else:
            log.debug(f"wpid=0x{self._device.wpid:04X}: feature 0x{self._feature_code:04X} not supported")
