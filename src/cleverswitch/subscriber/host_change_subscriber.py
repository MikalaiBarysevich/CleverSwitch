...
class HostChangeSubscriber(Subscriber):
    ...
    def notify(self, event) -> None:
        ...
        if event.function == 2:
            event_code = event.payload[2]
            if event_code != 0x01:  # ignore release, only process press
                return
        ...
        log.info(
            "Host change detected from %s (slot=%d) CID=0x%04X -> host %d",
            source.name,
            source.slot,
            cid,
            target_host + 1,
