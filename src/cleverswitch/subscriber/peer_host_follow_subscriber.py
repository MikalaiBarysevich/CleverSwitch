...
class PeerHostFollowSubscriber(Subscriber):
    ...
    def __init__(self, device_registry: LogiDeviceRegistry, topics: Topics, easy_switch_config: EasySwitchConfig):
    ...
    def notify(self, event) -> None:
        ...
        departed = self._find_by_slot(event.pid, event.slot)
        ...
        self._topics.hid_event.publish(
            HostChangeEvent(slot=departed.slot, pid=departed.pid, target_host=peer_host_index)
        )

    def _find_by_slot(self, pid: int, slot: int) -> LogiDevice | None:
        for entry in self._device_registry.all_entries():
            if entry.pid == pid and entry.slot == slot:
                return entry
        return None
