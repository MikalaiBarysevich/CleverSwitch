...
from ..hidpp.constants import CHANGE_HOST_FN_SET, FEATURE_CHANGE_HOST, FEATURE_REPROG_CONTROLS_V4, HOST_SWITCH_CIDS, SW_ID
...
class DivertedHostChangeSubscriber(Subscriber):
    ...
    def notify(self, event) -> None:
        ...
        log.info("Diverted host change detected from %s (slot=%d) CID=0x%04X -> host %d", source.name, source.slot, cid, target_host)
        ...
        for device in self._device_registry.all_entries():
            ...
            request_id = (change_host_idx << 8) | (CHANGE_HOST_FN_SET & 0xF0) | SW_ID
