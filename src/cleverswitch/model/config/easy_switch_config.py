...
class EasySwitchConfig:
    ...
    # guaranteed to match — hence per-role, not a single shared index. Learn each value once via
    # getHostInfo on the target machine — see config.example.yaml.
    #
    # 0-indexed internally (config.yaml's values are 1-based for humans, converted on load).
