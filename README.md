...
**Trade-off:** there is no way to tell an intentional Easy-Switch press apart from an ordinary RF dropout on the wire. To absorb brief dropouts, the follow is deferred by ~2 seconds - if the "departed" device reconnects within that window (measured micro-dropouts recover well inside it), the relay is cancelled and nothing moves. The cost is that a genuine switch is followed with the same ~2-second delay. A device that stays away longer - out of range, dead battery - still (incorrectly) sends its counterpart away. Unplugging the receiver looks the same, and will log a relay and fire `SWITCH` hooks even though the `CHANGE_HOST` command itself goes nowhere. Only enable this if you have exactly two machines and are comfortable with that trade-off.
