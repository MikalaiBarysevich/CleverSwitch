...
def test_make_logi_product_returns_none_when_change_host_not_supported(mocker, fake_transport):
    ...
    result = _make_logi_product(fake_transport, slot=1, long_msg=False, role="mouse", name="MX Master")
...
def test_make_logi_product_returns_logi_product_for_mouse(mocker, fake_transport):
    ...
    result = _make_logi_product(fake_transport, slot=2, long_msg=False, role="mouse", name="MX Master")
...
def test_make_logi_product_resolves_reprog_controls_for_keyboard(mocker, fake_transport):
    ...
    result = _make_logi_product(fake_transport, slot=1, long_msg=False, role="keyboard", name="MX Keys")
    assert result is not None
    assert result.change_host_feat_idx == 3
...
def test_make_logi_product_uses_long_msg_flag(mocker, fake_transport):
    calls = []
    mocker.patch(
        "cleverswitch.factory.resolve_feature_index",
        side_effect=lambda t, slot, feat, long=False: calls.append(long) or 5,
    )
    _make_logi_product(fake_transport, slot=1, long_msg=True, role="mouse", name="M")
...
def test_make_logi_product_name_is_preserved(mocker, fake_transport):
    ...
    result = _make_logi_product(fake_transport, slot=3, long_msg=False, role="mouse", name="MX Anywhere 3")
